...
function ProjectsPage() {
  ...
  const [projects, setProjects] = useState<
    { id: string; name: string; creator_id: string; creator_email: string }[]
  >([])
            ...
            {isLoading ? (
              ...
              >
                ...
                <tbody>
                    ...
                    <tr key={item.id} className="hover:bg-base-300">
                      ...
                      {item?.creator_id === user?.id && (
                        <td
                          ...
                          className="btn bg-[#7C4585] text-white"
                        >
                          <CiMenuKebab />
                        </td>
                      )}
                  ...
                  <ProjectDeleteConfirmation id={projectId} setProjects={setProjects} />
                </tbody>
              </motion.table>
            )}
          </div>
...
const SkeletonTable = () => {
  ...
  return (
    <table className="table animate-pulse">
      <thead>
        <tr>
          <th className="bg-gray-200 h-6 w-32 rounded"></th>
          <th className="bg-gray-200 h-6 w-32 rounded"></th>
          <th className="bg-gray-200 h-6 w-20 rounded"></th>
        </tr>
      </thead>
      <tbody>
        {rows.map((r) => (
    ...
    <dialog id="delete_modal" className="modal">
      <div className="modal-box">
        <div className="flex flex-col justify-center items-center">
          ...
          <button
            disabled={isDeleting}
            className="btn my-3 bg-red-500 text-white flex items-center gap-2"
